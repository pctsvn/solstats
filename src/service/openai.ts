...
export const analyzeTradingData = async (transactions: any[]) => {
    // const prompt = `
    ...
    //     Lưu ý: Nếu có giao dịch mua và bán, hãy lấy thời gian bán trừ cho thời gian mua cũ nhất nhưng trước lúc bán gần nhất, và trả về thêm kiểu thời gian ví dụ 1h
    ...
    //     - Loại: ${tx.flow === "in" ? "Mua" : "Bán"}
    //     - Số lượng: ${(tx.amount / 10 ** tx.token_decimals).toFixed(2)} ${tx.token_symbol}
    //     - Giá trị (USD): ${tx.value.toFixed(2)}
    //     - Thời gian: ${new Date(tx.time).toLocaleString()}`
    //         )
    //         .join("\n")}
